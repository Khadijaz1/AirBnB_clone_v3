...
test_db_storage.py'])
    ...
    def test_db_storage_module_docstring(self):
        ...
        self.assertIsNot(db_storage.doc, None,
                         "db_storage.py needs a docstring")
        self.assertTrue(len(db_storage.doc) >= 1,
                        "db_storage.py needs a docstring")

    def test_db_storage_class_docstring(self):
        """Test for the DBStorage class docstring"""
        self.assertIsNot(DBStorage.doc, None,
                         "DBStorage class needs a docstring")
        self.assertTrue(len(DBStorage.doc) >= 1,
                        "DBStorage class needs a docstring")

    def test_dbs_func_docstrings(self):
        """Test for the presence of docstrings in DBStorage methods"""
        for func in self.dbs_f:
            self.assertIsNot(func[1].doc, None,
                             "{:s} method needs a docstring".format(func[0]))
            self.assertTrue(len(func[1].doc) >= 1,
                            "{:s} method needs a docstring".format(func[0]))
